...
Deflection in particular is worth focusing on a bit, because of its simulation. The deflection field must perform work every frame, both in rendering all the affectors and in running the physics simulation. In addition to the field resolution we can also change the updates per second, which can also have significant performance impact. It too will lose some precision when it is lowered, so you will have to balance it based on your needs. Color and scale fields normally render when one of their affectors move, though if they have been set to restore over time they work more like deflection does.
